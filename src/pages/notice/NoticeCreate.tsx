...
import { NoticeStatus, NoticeCreate as NoticeCreateType } from '../../types/notice';
...
const NoticeCreate = () => {
    ...
    if (confirmed) {
      try {
        ...
        const noticeData: NoticeCreateType = {
          ...
          content,
          status,
          admin_user_id: 1  // 실제로는 로그인된 관리자 ID를 사용
        };
  ...
  return (
    ...
    style={{
      ...
      {/* Form */}
      <form onSubmit={handleSubmit} className="h-auto space-y-6 bg-white rounded-lg shadow p-6">
        {/* Title Input */}
        <div>
          <label htmlFor="title" className="block font-nanum text-sm font-medium text-gray-700 mb-1">
            제목
          </label>
          <input
            id="title"
        ...
        <div className="min-h-[50dvh] max-h-[45dvh] overflow-auto">
            <label htmlFor="content" className="block font-nanum text-sm font-medium text-gray-700 mb-1">
                내용
            </label>
        ...
        <div className="flex items-start justify-between border-t pt-6 absolute bottom-0 left-0 right-0 bg-white shadow-lg px-6 py-4">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {isLoading ? '저장 중...' : (status === 'PUBLISHED' ? '게시' : '임시저장')}
            </button>
          </div>
        </div>
      </form>
